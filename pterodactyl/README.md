...
- Ensure required DB/Redis endpoints are reachable from the node that hosts the server. Current API runtime needs `DATABASE_URL` and `PORT`; worker queue processing needs `REDIS_URL`.
